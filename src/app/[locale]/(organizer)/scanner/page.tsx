    ...
    <div className="space-y-6 max-w-5xl mx-auto animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/80 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-bold uppercase tracking-wider text-primary">
              Door Entry & Gate Operations
            </span>
            <Badge variant="success" size="sm">Gate Scanner Ready</Badge>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-foreground mt-1">
            Door Staff QR Check-In Console
          </h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            Validate attendee digital passes using HMAC-SHA256 signature verification, double-scan detection, and live perk unlocks.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground bg-card p-2 px-3 rounded-lg border border-border">
          ...
          <span>Cryptographic Pass Tamper Protection Active</span>
        </div>
      </div>
